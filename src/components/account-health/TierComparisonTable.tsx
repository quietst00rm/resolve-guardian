...
const FeatureCell = ({
  ...
  return <div className="flex items-center justify-center">
      {included ? <Check className="w-5 h-5 text-green-500" /> : <X className="w-4 h-4 text-muted-foreground/40" />}
    </div>;
};
  ...
  return <section className="py-20 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden lg:block">
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
            ...
            return <div key={catIdx}>
                  ...
                  {categoryFeatures.map((feature, featIdx) => <div key={featIdx} className="grid grid-cols-5 gap-0 border-t border-gray-200 hover:bg-blue-50 transition-colors duration-150">
                      {/* Feature Name */}
                      ...
                      <div className="p-5 flex items-center justify-center border-l border-gray-200">
                        <FeatureCell included={feature.guardian} detail={feature.details?.guardian} />
                      </div>

                      {/* Defender Column */}
                      <div className="p-5 flex items-center justify-center border-l border-gray-200">
                        <FeatureCell included={feature.defender} detail={feature.details?.defender} />
                      </div>

                      {/* Fortress Column */}
                      <div className="p-5 flex items-center justify-center border-l border-gray-200">
                        <FeatureCell included={feature.fortress} detail={feature.details?.fortress} />
                      </div>

                      {/* Empire Column */}
                      <div className="p-5 flex items-center justify-center border-l border-gray-200">
                        <FeatureCell included={feature.empire} detail={feature.details?.empire} />
                      </div>
                    </div>)}
                </div>;
          })}
            ...
            <div className="bg-gray-50 p-8 border-t border-gray-200 text-center">
              <button onClick={scrollToCalculator} className="px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl transition-colors duration-200 inline-flex items-center shadow-md hover:shadow-lg">
        ...
        <div className="lg:hidden space-y-4">
          ...
          return <div key={tier} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
                
                {/* Tier Header (Always Visible) */}
                <button onClick={() => setExpandedTier(isExpanded ? null : tier)} className="w-full p-6 flex items-center justify-between bg-gradient-to-r from-blue-600 to-blue-700 text-white">
                  ...
                  <ArrowRight className={`h-6 w-6 transition-transform duration-300 ${isExpanded ? 'rotate-90' : ''}`} />
                </button>

                {/* Expandable Feature List */}
                {isExpanded && <div className="p-6 space-y-4">
                    {categories.map((category, catIdx) => {
                const categoryFeatures = features.filter(f => f.category === category);
                return <div key={catIdx}>
                      ...
                      return <li key={featIdx} className="flex items-start">
                                  {included ? <Check className="h-5 w-5 text-green-500 mr-3 mt-0.5 flex-shrink-0" /> : <X className="h-5 w-5 text-gray-300 mr-3 mt-0.5 flex-shrink-0" />}
                                  <div>
                                    <span className="text-gray-700">{feature.feature}</span>
                                    {detail && <span className="text-gray-500 text-sm ml-2">({detail})</span>}
                                  </div>
                                </li>;
                    })}
                          </ul>
                    ...
                    <button onClick={scrollToCalculator} className="w-full mt-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-colors">
                      Find My Protection Tier
                    </button>
                  </div>}
              </div>;

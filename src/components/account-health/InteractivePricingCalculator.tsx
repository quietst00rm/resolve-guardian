...
export const InteractivePricingCalculator = () => {
  ...
  const nextStep = (step: number) => {
    if (step === 1) {
      ...
      setAnswers({
        ...answers,
        violationCount: violationInput.value
      });
    }
  ...
  return <section id="pricing-calculator" className="bg-background py-16">
      <div className="max-w-3xl mx-auto px-6">
        <div className="bg-card rounded-2xl shadow-2xl p-10 border border-border">
          ...
          {/* Progress Bar */}
          {!showResults && <div className="mb-8">
          ...
          <div className="relative min-h-[350px]">
            {/* Step 1: ASINs Count */}
            <div className={`transition-all duration-300 ${currentStep === 1 ? "opacity-100 translate-x-0 relative" : "opacity-0 absolute inset-0 pointer-events-none"}`}>
              <label className="text-2xl font-semibold text-foreground mb-6 block text-center">
                How many ASINs are you currently managing?
              </label>
              <div className="mb-8">
                <input type="number" id="asinCount" className="w-full p-4 text-lg border-2 border-border rounded-lg focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all" placeholder="Enter number of ASINs" min="0" onKeyPress={e => e.key === "Enter" && nextStep(1)} />
              </div>
            ...
            <div className={`transition-all duration-300 ${currentStep === 2 ? "opacity-100 translate-x-0 relative" : "opacity-0 absolute inset-0 pointer-events-none"}`}>
              <label className="text-2xl font-semibold text-foreground mb-6 block text-center">
                Have you been suspended before?
              </label>
              <div className="grid grid-cols-2 gap-4 mb-8">
                <button onClick={() => selectChoice("Yes")} className={`p-6 text-lg font-semibold border-2 rounded-lg transition-all ${answers.suspended === "Yes" ? "bg-primary text-white border-primary" : "bg-card border-border hover:border-primary hover:bg-blue-50"}`}>
                  Yes
                </button>
                <button onClick={() => selectChoice("No")} className={`p-6 text-lg font-semibold border-2 rounded-lg transition-all ${answers.suspended === "No" ? "bg-primary text-white border-primary" : "bg-card border-border hover:border-primary hover:bg-blue-50"}`}>
                  No
                </button>
              </div>
            ...
            <div className={`transition-all duration-300 ${currentStep === 3 ? "opacity-100 translate-x-0 relative" : "opacity-0 absolute inset-0 pointer-events-none"}`}>
              <label className="text-2xl font-semibold text-foreground mb-6 block text-center">
                How many monthly violations do you get on average?
              </label>
              <div className="mb-8">
                <input type="number" id="violationCount" className="w-full p-4 text-lg border-2 border-border rounded-lg focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all" placeholder="Enter average monthly violations" min="0" onKeyPress={e => e.key === "Enter" && nextStep(3)} />
              </div>
              <div className="flex gap-4">
                <Button onClick={previousStep} variant="outline" className="flex-1 py-6">
                  Back
            ...
            <div className={`transition-all duration-300 ${currentStep === 4 && !isCalculating ? "opacity-100 translate-x-0 relative" : "opacity-0 absolute inset-0 pointer-events-none"}`}>
              <label className="text-2xl font-semibold text-foreground mb-6 block text-center">
                What is your monthly revenue?
              </label>
              <div className="mb-8">
                <input type="text" id="monthlyRevenue" className="w-full p-4 text-lg border-2 border-border rounded-lg focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all" placeholder="$0" onChange={e => {
            ...
            {/* Calculating Screen */}
            {isCalculating && <div className="text-center py-12 animate-fade-in">
            ...
            {/* Results Screen */}
            {showResults && tierResult && <div className="text-center animate-fade-in">
                
                <h2 className="text-4xl font-bold text-foreground mb-2 my-[10px] mx-0 px-0 py-[5px]">{tierResult.name}</h2>
                ...
                <p className="text-lg text-muted-foreground mb-8">Based on your business profile, you fall into our Unlimited Protection<strong>{tierResult.name}</strong> tier

...
const tiers = [{
  name: "GUARDIAN",
  price: 349,
  features: ["Daily violation monitoring", "72-hour violation response time", "Unlimited violation handling", "Custom POA drafting & submission", "Weekly account health reports", "Email support with 48-hour response", "Account health score monitoring", "48-hour communication response time"]
}, {
  name: "DEFENDER",
  price: 899,
  inheritFrom: "GUARDIAN",
  features: ["48-hour violation response (2x faster)", "Annual compliance audit", "Priority violation queue", "24-hour communication response time"]
}, {
  name: "FORTRESS",
  price: 2199,
  inheritFrom: "DEFENDER",
  features: ["24-hour violation response (2x faster)", "6-hour communication response", "Dedicated account manager", "Slack channel + email support", "Twice-yearly compliance audits", "Quarterly strategy calls", "Daily check-ins when at risk", "Executive escalation handling"]
}, {
  name: "EMPIRE",
  price: 5999,
  inheritFrom: "FORTRESS",
  features: ["Same-day violation response", "2-hour communication response", "Senior dedicated account manager", "WhatsApp + Slack + Phone support", "Quarterly compliance audits", "Monthly strategy sessions"]
}];
const TierCard = ({
  tier
}: {
  ...
  return <div className="relative flex flex-col bg-white rounded-2xl border-2 border-gray-200 shadow-lg hover:shadow-2xl transition-all duration-300 p-8 min-h-[520px]">
      {/* Tier Name */}
      ...
      {"inheritFrom" in tier && tier.inheritFrom && <p className="text-gray-600 mb-6 min-h-[60px] leading-relaxed">
      ...
      <ul className="space-y-3 mb-4 flex-grow">
        {displayedFeatures.map((feature, idx) => <li key={idx} className="flex items-start">
      ...
      {hasMoreFeatures && <button onClick={() => setShowAllFeatures(!showAllFeatures)} className="text-blue-600 hover:text-blue-700 text-sm font-medium flex items-center gap-1 mb-8 transition-colors">
      ...
      <div className="mt-auto">
        <Button onClick={scrollToCalculator} className="w-full py-4 px-6 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl transition-colors duration-200 flex items-center justify-center group shadow-md hover:shadow-lg h-auto">
...
export const TierOverview = () => {
  ...
  return <section id="tier-overview" className="py-20 bg-gradient-to-b from-gray-50 to-white">

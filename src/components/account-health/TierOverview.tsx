...
const tiers = [
  ...
  },
  {
    ...
    coreFeatures: [
      ...
      "WhatsApp + Slack + executive hotline",
...
const TierCard = ({ tier }: { tier: typeof tiers[0] }) => {
  ...
  return (
    <div
      className={`relative bg-card rounded-2xl p-8 transition-all duration-400 hover:-translate-y-2 min-h-[560px] flex flex-col ${
        tier.popular 
    ...
    >
      ...
      <div className="mb-4">
        ...
        <ul className="space-y-3">
          {tier.coreFeatures.map((feature, idx) => (
      ...
      <Collapsible open={isExpanded} onOpenChange={setIsExpanded} className="mb-4">
        <CollapsibleContent className="space-y-3">
          ...
          <ul className="space-y-3">
            {tier.additionalFeatures.map((feature, idx) => (
              <li key={idx} className="flex items-start gap-3">
                <Check className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                <span className="text-[14px] font-medium text-foreground leading-relaxed">
                  {feature}
      ...
      <Button
        ...
        className={`w-full h-[52px] text-base font-semibold rounded-lg transition-all duration-300 mt-auto ${
          tier.popular

  ...
  const steps = [
    {
      ...
      expandedContent:
        "Our team logs into your Seller Central account daily to conduct thorough manual reviews of your account health, performance metrics, notifications, and listings. We identify potential violations and issues within 24 hours, giving you the fastest possible response time to protect your account.",
    },
    {
      number: "02",
      icon: FileText,
      title: "Approval-Ready POAs & Responses",
    ...
    },
    {
      ...
      expandedContent:
        "Unlike competitors who make you handle submissions, we manage the entire process. You receive the POA for review and approval, then we handle all submissions, follow-up, and communication with Amazon. You stay informed via email updates while we do the work.",
    },
  ];
  return (
    <section id="how-it-works" className="bg-card py-24">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="text-center mb-20">
          <h2 className="text-5xl font-bold text-foreground mb-4">Set It. Forget It. We Handle Everything.</h2>
          <p className="text-xl text-muted-foreground">Three-step protection that runs on autopilot</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 lg:gap-12 relative">
          ...
          <div
            className="hidden md:block absolute top-16 left-0 right-0 h-0.5 bg-gradient-to-r from-primary via-primary to-primary"
            style={{
              width: "calc(100% - 200px)",
              marginLeft: "100px",
              background:
                "repeating-linear-gradient(to right, hsl(var(--primary)) 0, hsl(var(--primary)) 10px, transparent 10px, transparent 20px)",
            }}
          >
            {/* Animated dot */}
            <div
              className="absolute top-1/2 -translate-y-1/2 w-3 h-3 bg-primary rounded-full animate-pulse"
              style={{
              ...
              <div key={index} className="relative">
                ...
                <div className="text-center space-y-4">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`h-4 w-4 transition-all duration-300 ${isExpanded ? "rotate-180" : ""} group-hover:translate-y-0.5`}
                    />
                  </button>

                  {/* Expanded Content */}
                  <div
                    className={`overflow-hidden transition-all duration-500 ease-in-out ${isExpanded ? "max-h-[600px] opacity-100 mt-6" : "max-h-0 opacity-0 mt-0"}`}
                  >
                    <div className="relative bg-gradient-to-br from-primary/5 to-blue-500/5 border border-primary/20 p-6 rounded-xl text-left shadow-lg">
                      ...
                      <p className="text-sm text-foreground/80 leading-relaxed pl-3 mb-4">{step.expandedContent}</p>
